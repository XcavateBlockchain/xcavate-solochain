...
```sh
./target/release/xcavate-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/xcavate-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/xcavate-node -ldebug --dev
```
...
$ ./target/release/xcavate-node --dev --base-path ./my-chain-state/
