...
use sp_runtime::{
    traits::{AccountIdConversion, BlockNumberProvider, CheckedAdd, One},
    Permill,
};
...
pub mod pallet {
    ...
    pub struct RegionInfo<AccountId, Balance, BlockNumber, NftCollectionId> {
        /// NFT collection identifier linked to the region.
        pub collection_id: NftCollectionId,
        // Duration (in blocks) that a property listing remains valid within the region.
        pub listing_duration: BlockNumber,
    ...
    #[derive(Encode, Decode, Clone, PartialEq, Eq, MaxEncodedLen, RuntimeDebug, TypeInfo)]
    #[scale_info(skip_type_params(T))]
    pub struct LawyerInfo<Balance> {
        /// Region ID where the lawyer is registered.
        pub region: RegionId,
        /// Collateral deposit locked by the lawyer to participate in cases.
        pub deposit: Balance,
        /// Number of currently active cases assigned to this lawyer.
    ...
    pub enum HoldReason {
        ...
        /// Funds are held for a lawyer to operate in a region.
        #[codec(index = 3)]
    ...
    pub trait Config: frame_system::Config {
        ...
        #[pallet::constant]
        type MaxTaxPercent: Get<Permill>;
    ...
    pub type RealEstateLawyer<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        T::AccountId,
        LawyerInfo<<T as pallet::Config>::Balance>,
        OptionQuery,
    >;
    ...
    pub enum Event<T: Config> {
        ...
        ListingDurationChanged { region_id: RegionId, listing_duration: BlockNumberFor<T> },
        ...
        LawyerRegistered { lawyer: T::AccountId, region_id: RegionId, deposit: T::Balance },
        /// Lawyer has been unregistered.
        LawyerUnregistered { lawyer: T::AccountId },
        /// Active cases of a lawyer have been updated.
        LawyerActiveCasesUpdated { lawyer_account: T::AccountId, new_active_cases: u32 },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_new_region(
            ...
            ensure!(tax <= T::MaxTaxPercent::get(), Error::<T>::TaxTooHigh);

            // Create a new NFT collection for the region
        ...
        pub fn adjust_listing_duration(
            ...
            // Update the region's listing duration
            ...
            Self::deposit_event(Event::<T>::ListingDurationChanged { region_id, listing_duration });
            Ok(())
        }
        ...
        pub fn adjust_region_tax(
            ...
            // Validate new tax
            ensure!(new_tax <= T::MaxTaxPercent::get(), Error::<T>::TaxTooHigh);

            // Update the region's tax rate
            RegionDetails::<T>::try_mutate(region_id, |maybe_region| {
                let region = maybe_region.as_mut().ok_or(Error::<T>::RegionUnknown)?;
        ...
        pub fn create_new_location(
            ...
            let signer = T::PermissionOrigin::ensure_origin(origin, &Role::RegionalOperator)?;
            // Prevent duplicate location registrations
            ensure!(
            ...
            RegionDetails::<T>::try_mutate(region_id, |maybe_region| {
                ...
                // Hold the required deposit for the new location
                T::NativeCurrency::hold(
                    &HoldReason::RegionDepositReserve.into(),
                    &signer,
                    deposit_amount,
        ...
        pub fn bid_on_region_replacement(
            ...
            RegionReplacementAuctions::<T>::try_mutate(region_id, |maybe_auction| {
                let auction = maybe_auction.get_or_insert_with(|| {
                    new_auction = true;
                    // Calculate the minimum deposit based on the number of locations
                    ...
                    minimum_deposit.saturating_accrue(location_deposits);
                    RegionAuction {
                        highest_bidder: None,
        ...
        pub fn register_lawyer(origin: OriginFor<T>, region: RegionId) -> DispatchResult {
            let signer = T::PermissionOrigin::ensure_origin(origin, &Role::Lawyer)?;
            // Validate region and registration status
            ...
            // Hold the required deposit for the lawyer
            ...
            // Register the lawyer
            let lawyer_info = LawyerInfo { region, deposit: deposit_amount, active_cases: 0 };
        ...
        #[pallet::weight(<T as pallet::Config>::WeightInfo::unregister_lawyer())]
        pub fn unregister_lawyer(origin: OriginFor<T>) -> DispatchResult {
            let signer = T::PermissionOrigin::ensure_origin(origin, &Role::Lawyer)?;
            let lawyer_info =
                RealEstateLawyer::<T>::get(&signer).ok_or(Error::<T>::NoPermission)?;
            // Ensure the lawyer has no active cases
            ensure!(lawyer_info.active_cases.is_zero(), Error::<T>::LawyerStillActive);

            // Release the lawyer's deposit
            ...
            // Remove the lawyer's registration
            RealEstateLawyer::<T>::remove(&signer);
            Self::deposit_event(Event::<T>::LawyerUnregistered { lawyer: signer });
            Ok(())
        }
    }
...
/// Trait for managing lawyer active case counts.
pub trait LawyerManagement<AccountId> {
    type LawyerInfo;

    // Increments the active case count for a lawyer.
    fn increment_active_cases(lawyer: &AccountId) -> DispatchResult;

    // Decrements the active case count for a lawyer.
    fn decrement_active_cases(lawyer: &AccountId) -> DispatchResult;

    // Retrieves the information of a registered lawyer.
    fn get_lawyer_info(lawyer: &AccountId) -> Option<Self::LawyerInfo>;
}
...
pub trait RegionTrait {
    ...
    // Checks if a location is registered within a specific region.
    fn location_registered(region_id: RegionId, location: Self::LocationIdentifier) -> bool;
}

impl<T: Config> LawyerManagement<T::AccountId> for Pallet<T> {
    type LawyerInfo = LawyerInfo<<T as pallet::Config>::Balance>;

    /// Increments the active case count for a lawyer.
    fn increment_active_cases(lawyer: &T::AccountId) -> DispatchResult {
        RealEstateLawyer::<T>::try_mutate(lawyer, |maybe_lawyer_info| {
            let lawyer_info = maybe_lawyer_info.as_mut().ok_or(Error::<T>::LawyerNotRegistered)?;
            lawyer_info.active_cases =
                lawyer_info.active_cases.checked_add(1).ok_or(Error::<T>::ArithmeticOverflow)?;
    ...
    /// Decrements the active case count for a lawyer.
    fn decrement_active_cases(lawyer: &T::AccountId) -> DispatchResult {
        RealEstateLawyer::<T>::try_mutate(lawyer, |maybe_lawyer_info| {
            let lawyer_info = maybe_lawyer_info.as_mut().ok_or(Error::<T>::LawyerNotRegistered)?;
            lawyer_info.active_cases =
                lawyer_info.active_cases.checked_sub(1).ok_or(Error::<T>::ArithmeticUnderflow)?;
            Self::deposit_event(Event::LawyerActiveCasesUpdated {
                lawyer_account: lawyer.clone(),
                new_active_cases: lawyer_info.active_cases,
            });
    ...
    /// Retrieves the information of a registered lawyer.
    fn get_lawyer_info(lawyer: &T::AccountId) -> Option<Self::LawyerInfo> {
        RealEstateLawyer::<T>::get(lawyer)
    }
}

impl<T: Config> RegionTrait for Pallet<T> {
    type Info = RegionInfoOf<T>;
    type LocationIdentifier = LocationId<T>;
